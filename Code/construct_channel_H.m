function H_eff = construct_channel_H(bs_pos, user_pos, scatter_pos, ...
                                     bs_input, ue_rotations, F_analog, U_user, ...
                                     sim_param, nlos_phase_bank)
% 功能:
%   基于给定的几何位置、天线旋转状态 (BS/UE) 和波束赋形向量，
%   计算包含大尺度衰落、天线增益、极化失配和多径效应的等效标量信道。
% 输入:
%   bs_input - [3xM] 欧拉角 或 [3x3xM] 旋转矩阵
%   F_analog - 发射端极化状态
%   U_user   - 接收端极化状态
% 输出:
%   H_eff    - [M x K] 复数信道增益矩阵
    [~, M] = size(bs_pos);
    [~, K] = size(user_pos);
    [~, L] = size(scatter_pos);
    ...
    for k = 1:K
        ...
        for m = 1:M
            ...
            term_pol_los = (u_vec)' * (Q_los * M_mat * P_los * f_vec);
            
            h_los = pl_amp_los *  sqrt(G_val) * term_pol_los;
            ...
            if L > 0
                for l = 1:L
                    ...
                    J_mat = [sqrt_co, sqrt_cx*rand_phase_1; 
                             sqrt_cx*rand_phase_2, sqrt_co] * phase_prop;
                    ...
                    term_pol_nlos = (u_vec)' * (Q_nlos * J_mat * P_nlos * f_vec);
                    h_nlos_sum = h_nlos_sum + pl_amp_nlos  * sqrt(G_nlos) * term_pol_nlos;
                end
            end
            
            h_vec_k(m) = h_los + h_nlos_sum;
        end
        H_eff(:, k) = h_vec_k;
    end
end